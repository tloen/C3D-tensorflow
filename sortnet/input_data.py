...
def get_all_data(filename):
  ...
  all consecutive frames as a list of np arrays '''
  ret_arr = []
  image_names = []
  s_index = 0
...
def read_clip(filename, batch_size, start_pos=-1, num_frames_per_clip=16, crop_size=224, shuffle=False):
  ...
  # pad (duplicate) data if less than batch_size
  valid_len = len(data)
...
def read_clip_full(dirname, crop_size=224):
  ...
  np_mean = np.repeat(np_mean, 2, 1)
  np_mean = np.repeat(np_mean, 2, 2)
  np_mean = np.mean(np_mean, axis=0, keepdims=True)
  np_mean = np_mean.reshape([1, crop_size, crop_size, 3])
    
  tmp_data, img_names = get_all_data(dirname)
  if(len(tmp_data)!=0):
    for j in xrange(len(tmp_data)):
      img = Image.fromarray(tmp_data[j].astype(np.uint8))
      if(img.width>img.height):
      ...
      crop_x = int((img.shape[0] - crop_size)/2)
      crop_y = int((img.shape[1] - crop_size)/2)
      img = img[crop_x:crop_x+crop_size, crop_y:crop_y+crop_size,:] - np_mean[0]
      data.append(img)
  np_arr_data = np.array(data).astype(np.float32)
  return np_arr_data, img_names
